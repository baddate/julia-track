...
decipher function
"""
function decipher_rotate(shift, cipher)
    table = "abcdefghijklmnopqrstuvwxyz"
    plain = ""
    if typeof(cipher) == Char
        pos = findfirst(cipher, table)-shift > 0 ? findfirst(cipher, table)-shift : findfirst(cipher, table)-shift+26
        return table[pos]
    end
    for c in cipher
        if findfirst(c, table) === nothing && findfirst(c, uppercase(table)) === nothing
        ...
        pos = findfirst(lowercase(c), table)-shift > 0 ? findfirst(lowercase(c), table)-shift : findfirst(lowercase(c), table)-shift+26
        plain *= isuppercase(c) ? uppercase(table[pos]) : table[pos]
    end
    return plain
end
